...
def create_llm(model_path = config["model_path"]["large"], model_config = config.get("model_config", {})):
    llm_params = {}
    if "gpu_layers" in model_config:
        llm_params["n_gpu_layers"] = model_config["gpu_layers"]
    if "context_length" in model_config:
        llm_params["n_ctx"] = model_config["context_length"]
    if "temperature" in model_config:
        llm_params["temperature"] = model_config["temperature"]
    if "max_new_tokens" in model_config:
        llm_params["max_tokens"] = model_config["max_new_tokens"]
    llm_params["model_path"] = model_path
    llm_params["verbose"] = True  # Good for debugging
